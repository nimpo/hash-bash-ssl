...
function getSubject() { # Reads in HEX string and finds Subject DN and spits out inner HEX string Assuming an X.509
  getContents |getContents 1 | sed -e 's/a00302010[012]//' |getContents 5
}

function getIssuer() { # Reads in HEX string and finds Subject DN and spits out inner HEX string Assuming an X.509
  getContents |getContents 1 | sed -e 's/a00302010[012]//' |getContents 3
}
...
do
  case "$1" in
    -old)    OLD="y" ; shift ;;
    -issuer) ISSUER="y" ; shift ;;
    -subject) SUBJECT="y" ; shift ;;
    *) [ -e "$1" ] && PEMS+=("$1") || exit 1 ; shift ;;
  esac
done
[ "$ISSUER" = "" ] && SUBJECT="y"
...
for PEM in ${PEMS[@]}
do
  [ "$ISSUER" = "y" ] && cat "$PEM" | getDERfromPEM | getIssuer | ( [ "$OLD" = "y" ] && ASN1wrap "30" ||  canonicalizeDN ) | hextochar |digest "$OLD" |sed -e 's/\(..\)\(..\)\(..\)\(..\).*/\4\3\2\1/'
  [ "$SUBJECT" = "y" ] && cat "$PEM" | getDERfromPEM | getSubject | ( [ "$OLD" = "y" ] && ASN1wrap "30" ||  canonicalizeDN ) | hextochar |digest "$OLD" |sed -e 's/\(..\)\(..\)\(..\)\(..\).*/\4\3\2\1/'
done
